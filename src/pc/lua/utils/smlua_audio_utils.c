...
struct ModAudio* audio_load_internal(const char* filename, bool isStream) {
    ...
    // load audio
    FILE *f = fopen(modFile->cachedPath, "rb");
    if (!f) {
    ...
    if ((fstat(fd, &stbuf) != 0) || (!S_ISREG(stbuf.st_mode))) {
        LOG_ERROR("failed to load audio file '%s': file abnormal", filename);
        return NULL;
    }
    ...
    if (!buffer) {
        fclose(f);
        LOG_ERROR("failed to load audio file '%s': cannot allocate buffer of size: %d", filename, (int)size);
        return NULL;
    }

    // read the audio buffer
    if (fread(buffer, 1, (size_t)size, f) < size) {
        free(buffer);
        fclose(f);
        LOG_ERROR("failed to load audio file '%s': cannot read audio buffer of size: %d", filename, (int)size);
        return NULL;
    }
    fclose(f);
    ...
    ma_result result = ma_decoder_init_memory(buffer, (int)size, NULL, decoder);
